...
if ($id && $status && in_array($status, $statusPermitidos)) {
    try {
        $sql = "UPDATE materiais SET status = ? WHERE id = ? AND usuario_id = ?";
        ...
        if ($stmt->execute([$status, $id, $usuario_id])) {
            if ($stmt->rowCount() > 0) {
                ...
                echo json_encode(['success' => false, 'message' => 'Material não encontrado ou você não tem permissão para alterá-lo.']);
            }
        } else {
